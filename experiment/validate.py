...
from data import mscmr as mscmr
from util.loss import *



model_type = "unet"#可替换为其他net
fold = 5  # 1 2 3 4 5
augdata = "T2" #‘DE'，’C0‘
...
root_path = '/Users/software/codes ZMIC/U-net seg/'
...
palette = mscmr.palette
custom_palette = mscmr.custom_palette
num_classes = mscmr.num_classes
...
#输出运行日志（比print线程稳定）
    ...
    for slice, (input, mask,  file_name) in tqdm(enumerate(val_loader, 1)):
        ...
        m1 = helper.array_to_img(np.expand_dims(m1, 2))

        # gt
        gt = helper.onehot_to_mask(np.array(mask.squeeze()).transpose([1, 2, 0]), palette)
        gt = helper.array_to_img(gt)

        # pred
        save_pred = helper.onehot_to_mask(np.array(pred.squeeze()).transpose([1, 2, 0]), palette)
        save_pred_png = helper.array_to_img(save_pred)
        save_pred_color = helper.onehot_to_mask(np.array(pred.squeeze()).transpose([1, 2, 0]), custom_palette)
        save_pred_png_color = helper.array_to_img(save_pred_color)
