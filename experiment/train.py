...
def main(Baseline, fold, loss_name, train_path, val_path):
    # 定义网络
    net = Baseline(num_classes=mscmr.num_classes).cuda()
    ...
    train_set = mscmr.MSCMR2019(train_path, 'train', fold, joint_transform=None, roi_crop=None,
                                    center_crop=center_crop,
                                    transform=input_transform, target_transform=target_transform)
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=batch_size)
    val_set = mscmr.MSCMR2019(val_path, 'val', fold,
    ...
    # 定义损失函数
    if loss_name == 'dice':
        criterion = SoftDiceLoss(mscmr.num_classes).cuda()   #使用softdiceloss
    if loss_name =='bce':
        criterion - BCE_Dice_Loss(mscmr.num_classes).cuda()   #使用bceloss
...
def train(train_loader, val_loader, net, criterion, optimizer, scheduler, warm_scheduler, early_stopping, num_epoches,
    ...
    for epoch in range(1, num_epoches + 1):
        st = time.time()
        train_class_dices = np.array([0] * (mscmr.num_classes - 1), dtype=np.float)
        val_class_dices = np.array([0] * (mscmr.num_classes - 1), dtype=np.float)
        val_dice_arr = []
        train_losses = []
        val_losses = []
        ...
        for batch, (input, mask, __) in enumerate(train_loader, 1):
            ...
            class_dice = []
            for i in range(1, mscmr.num_classes):
        ...
        std = (np.std(val_dice_arr[:, 1:2]) + np.std(val_dice_arr[:, 2:3]) + np.std(val_dice_arr[:, 3:4])) / mscmr.num_classes
        val_class_dices = val_class_dices / val_batch

        val_mean_dice = val_class_dices.sum() / val_class_dices.size
        organ_mean_dice = (val_class_dices[0] + val_class_dices[1] + val_class_dices[2]) / mscmr.num_classes
...
if __name__ == '__main__':
    ...
    if model_type == "unet":
        ...
        from networks.resUnet import Baseline
