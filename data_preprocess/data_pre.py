...
#按照图片格式分别命名
    ...
    for n in tqdm(range(len(gt_namelist))):
        ...
        for i in range(c):
            ...
            # 保存png格式
            if to_png:
                png = helper.array_to_img(np.expand_dims(npy, axis=2))
                # mask_png = helpers.array_to_img(np.expand_dims(mask, axis=2))

                mask_png_color = helper.mask_to_onehot(np.expand_dims(mask, axis=2), palette)
                mask_png_color = helper.onehot_to_mask(mask_png_color, custom_palette)
                mask_png_color = helper.array_to_img(mask_png_color)
        ...
        for item in tqdm(items):
            ...
            for angle in angles:
                ...
                if to_png:
                    ...
                    png_im = helper.array_to_img(np.expand_dims(img_, axis=2))
                    png_gt = helper.array_to_img(np.expand_dims(gt_, axis=2))
...
if __name__ == '__main__':
    ...
    # 1.3d nii 图像转为 2d 切片
    types = ['DE', 'C0', 'T2']
    # nii_to_npy(r'/Users/software/codes ZMIC/U-net seg', r'../MSCMR2019', modal_type=types[2])


    # 3. 数据集增广
    data_augmentation(r'/Users/software/codes ZMIC/U-net seg/MSCMR2019',
                      r'/Users/software/codes ZMIC/U-net seg', modal_type=types[2])
